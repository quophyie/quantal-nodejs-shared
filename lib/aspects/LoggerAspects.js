...
class LoggerAspects {
   ...
   * @param {boolean} options.shouldPrintErrorInConsole - a boolean indicating whether errors should be printed to the console
   */
  constructor (logger, events, options = {mustSupplyTraceId: true, mustSupplyEvent: true, shouldPrintErrorInConsole: false}) {
    ...
    if (!options.events) throw new CommonErrors.IllegalArgumentError(`parameter 'events' is required and must be an object that specifies all supported events as properties of the event object`)
    _logger = logger
    _mustSupplyEvent = options.mustSupplyEvent
    _mustSupplyTraceId = options.mustSupplyTraceId
    _shouldPrintErrorInConsole = options.shouldPrintErrorInConsole
    _events = events
