...
const verifyEvent = (event) => {
  ...
  const foundEvent = Object.values(_events).find((ev) => event === ev)

  if (!foundEvent) {
    const err = new EventNotFoundError(`the event '${event}' was not found in the events object. 
    Please add the '${event}' to the events object that is passed to LoggerAspect constructor`)
    debug(err)
    printErrorsInConsole(err)
    _logger.throwing(err)
  }
...
class LoggerAspects {
  ...
  constructor (logger, events, mustSupplyEvent = true, shouldPrintErrorInConsole = false) {
    ...
    if (!events) throw new CommonErrors.IllegalArgumentError(`parameter 'events' is required and must be an object that specifies `)
    _logger = logger
