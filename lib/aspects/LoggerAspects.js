...
const CommonErrors = require('quantal-errors')
let _mustSupplyEvent = false, _shouldPrintErrorInConsole = true
let _logger = null

// import {beforeMethod} from 'aspect.js'
const beforeMethod = require('aspect.js').beforeMethod
const printConsoleInConsole = (err) => {
...
class LoggerAspects {
   ...
   */
  constructor (logger, mustSupplyEvent = true, shoupPrintErrorInConsole = true) {
  ...
  createAndApplyLogObject (meta) {
    ...
    if (meta.method) {
      if (typeof meta.method.args[0] === 'string') {
        if (_mustSupplyEvent && meta.method.args.length < 2) {
