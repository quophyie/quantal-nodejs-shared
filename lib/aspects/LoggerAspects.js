    ...
    if (meta.method) {
      if (typeof meta.method.args[0] !== 'object') {
        if (_mustSupplyEvent && typeof meta.method.args.length < 2) {
          const err = new CommonErrors.IllegalArgumentError('an event must be supplied as the 1st argument to the log function ')
          _logger.getLoggingFramework().error(err)
        }
        ...
        if (_mustSupplyEvent && !meta.method.args[0].event) {
          const err = new CommonErrors.IllegalArgumentError('an event property must be supplied on the object which is supplied as the 1st param of the log function')
          _logger.getLoggingFramework().error(err)
        }
